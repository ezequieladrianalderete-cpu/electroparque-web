...
export default async function BuscarPage({ searchParams }: Props) {
  ...
  let offers: OfferLike[] = [];
  const safeQ = q?.replace(/[,()]/g, ' ').trim();
  if (safeQ) {
    const supabase = await createClient();
    const [{ data }, { data: offersData }] = await Promise.all([
      supabase.from('products').select('*, category:categories(id,name,slug), images:product_images(*)').eq('is_active', true).or(`name.ilike.%${safeQ}%,short_description.ilike.%${safeQ}%`),
      supabase.from('offers').select('*').eq('is_active', true),
    ]);
    products = (data || []) as unknown as Product[];
    offers = (offersData || []) as unknown as OfferLike[];
  }
