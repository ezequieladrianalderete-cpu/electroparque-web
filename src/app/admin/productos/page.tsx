...
export default function ProductosListPage() {
  ...
  const remove = async (id: string) => { if (confirm('¿Eliminar producto?')) { await supabase.from('products').delete().eq('id', id); await load(); } };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b px-6 py-4 flex items-center gap-3">
        ...
        <h1 className="font-bold text-lg flex-1">Productos ({products.length})</h1>
        <Link href="/admin/productos/nuevo" className="bg-ep-navy text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2"><Plus className="w-4 h-4"/>Nuevo</Link>
      </div>
      <div className="max-w-5xl mx-auto p-6">
        {products.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            <p className="text-lg mb-2">No hay productos</p>
            <Link href="/admin/productos/nuevo" className="text-ep-navy font-semibold">Crear el primero →</Link>
          </div>
        ) : (
              ...
              return (
                <div key={p.id} className="bg-white rounded-xl border p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-bold text-ep-navy mt-1">${Number(p.price).toLocaleString('es-AR')}</p>
                  </div>
                  <button onClick={() => toggle(p.id, p.is_active)} className={`p-2 rounded-lg ${p.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`} title={p.is_active ? 'Activo' : 'Inactivo'}>
